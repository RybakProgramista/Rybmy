...
export class ShopComponent{
  ...
  getItemStateFromNumber(state : number) : itemState{
    switch(state){
      case 1:
        return "NotBought";
      case 2:
      ...
      case 3:
        return "Equipped";
      default:
        console.error("ZŁY STATUS PRZEDMIOTU");
        return "NotBought";
  ...
  changeItem(type : equipmentType, val : number){
    ...
    this._service.getItem(type.toString(), this.currIds.get(type) ?? 0).subscribe(
      e => {
        this.currItems.set(type, new BaseItem(e.nazwa, e.wytrzymalosc, e.cena, "NotBought" , ""));//FUCKED UP SHIT
      }
    )
  }
