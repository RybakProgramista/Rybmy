...
const querys = {
    buyEquip: {
        ...
        wedka: 'UPDATE `gracz`, `wedka` SET `gracz`.`pieniadze`= CASE WHEN `gracz`.`pieniadze` >= `wedka`.`cena` THEN `gracz`.`pieniadze` - `wedka`.`cena` ELSE `gracz`.`pieniadze` END, `gracz`.`wedka` = CASE WHEN `gracz`.`pieniadze` >= `wedka`.`cena` THEN concat(`gracz`.`wedka`, ?) ELSE `gracz`.`wedka` END WHERE `gracz`.`idGracz`=? AND `wedka`.`id`=?;',
    },
    possibilityToBuy: {
